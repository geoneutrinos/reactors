...
const DetectorCircles = React.memo(function DetectorCircles({detectors, changeDetector}){
    const color = '#9d00ff'
    return detectors.map((detector) =>{
        const DetectorPopup = (
            <Popup>
                ...
                <button onClick={() => changeDetector({ current: detector.name, lat: detector.lat, lon: detector.lon, elevation: detector.elevation })}>Place Detector Here</button>
            </Popup>)
...
export function NuMap({ onMousemove, detector, changeDetector, cores, detectorList }) {
    const mapStyle = {
        height: "100%",
        cursor: "crosshair"
    };
        ...
        contextmenuItems: [
            {
                ...
                callback: (e) => changeDetector({ current: "custom", elevation: 0, lat: e.latlng.lat, lon: e.latlng.lng })
            },
            '-',
            {
                text: 'Follow Cursor',
        ...
        >
            ...
            <LayersControl position="topright">
                ...
                <LayersControl.Overlay checked name='Detector Locations'>
                    <LayerGroup>
                        <DetectorCircles detectors={detectorList} changeDetector={changeDetector} />
                    </LayerGroup>
                </LayersControl.Overlay>
            </LayersControl>
        </Map>)
