...
type Oscilation = OscilationParams & OscilationFunctions;
...
interface OsilationParamsAction{
  arg: keyof VariableOscilationParams
  value: number
}
...
export const oscilationReducer = (state:Oscilation, action:OsilationParamsAction): Oscilation => {
  ...
  switch (action.arg) {
    ...
    case "s2t13Inverted":
      {
        let s2t13Inverted = action.value;
        oscilation.s2t13Inverted = s2t13Inverted;
        oscilation.c4t13Inverted = (1 - s2t13Inverted) ** 2;
        oscilation.s22t13Inverted =
          4 * s2t13Inverted * (1 - s2t13Inverted);
      }
      break;

    case "dmsq32Normal":
      {
        let dmsq32Normal = action.value;
    ...
    case "dmsq32Inverted":
      {
        let dmsq32Inverted = action.value;
        let { dmsq21 } = oscilation;
  ...
  oscilation.normalNeutrinoOscilationSpectrum = memoize((dist:number):Float64Array => {
    const oscspec = new Float64Array(1000);
    oscspec.fill(0);

    return oscspec.map((value, index) => oscilation.normalNeutrinoFlavor((index+1)*0.01, dist))
  })
  ...
  oscilation.invertedNeutrinoOscilationSpectrum = memoize((dist:number):Float64Array => {
    const oscspec = new Float64Array(1000);
    oscspec.fill(0);

    return oscspec.map((value, index) => oscilation.invertedNeutrinoFlavor((index+1)*0.01, dist))
  })
  return oscilation
}
